...
async function validateForm(inputs: Partial<Inputs>): Promise<ValidationResult> {
    const errors: ValidationError[] = [];

    // Check that all fields are present
    const requiredFields: (keyof Inputs)[] = ['currentAccount', 'name', 'lastName', 'age', 'email', 'phone'];
    ...
    if (!phoneRegex.test(inputs.phone!)) {
        errors.push({
            field: 'phone',
            message: 'Numero di telefono non valido (deve essere un numero italiano di 10 cifre)'
        });
    }
    ...
    if (typeof inputs.currentAccount !== 'string' || inputs.currentAccount.trim() === '') {
        errors.push({
...
export async function validateFormWithDelay(inputs: Partial<Inputs>): Promise<ValidationResult> {
    return new Promise((resolve, reject) => {
        setTimeout(async () => {
            try {
                ...
                reject(error instanceof Error ? error : new Error('Si è verificato un errore durante la validazione del form'));
            }
        }, 1000);
    });
}
    ...
    return new Promise((resolve, reject) => {
        setTimeout(async () => {
            try {
                ...
                reject(error instanceof Error ? error : new Error('Si è verificato un errore durante l\'invio del form'));
            }
        }, 1000);
    });
}
